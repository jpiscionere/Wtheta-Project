...
int main()
{
        double chi2_fof(int ,double ,double ,double ,double ,double );
        int i;
        double chi2;

        for(i=0;i<9;i++){
                chi2=chi2_fof(18, i,12.7053039145 ,  1.14827081243 ,  0.255258314714 , 1.31705259849); 
                fprintf(stderr,"%d %lf\n",i,chi2);
        }
return 0;
...
double chi2_fof(int lum_sample,double box_choose,double logM1,double alpha,double gamma,double fgal)
{
	...
	double logM0=9.81,siglogM=0.19;
	int box;
	//get_Mmin parameters
	...
	//halobias chars	
	snprintf(halobias_path,MAXLEN,"/home1/02497/jenpi/Source/halobias_fof_nfw_ff");
...
//	snprintf(file_tag,MAXLEN,"tag");
	if(sizeof(file_tag) > MAXLEN)
	  return BAD_VALUE;
	//makemock chars
	snprintf(makemock_path,MAXLEN,"/home1/02497/jenpi/Source/makemock_double");
	snprintf(map_directory,MAXLEN,"/scratch/02497/jenpi/Files");
//	snprintf(map_file,MAXLEN,"%s/lss_geometry.dr72.stripe_trim.pix",map_directory);
	...
	//covar chars
	snprintf(data_directory,MAXLEN,"/scratch/02497/jenpi/Files");	
	...
	snprintf(stomp_path,MAXLEN,"/home1/02497/jenpi/Source/astro-stomp-read-only/examples/stomp_galaxy_autocorrelation");
	snprintf(halo_directory,MAXLEN,"/scratch/02497/jenpi/halo_files/%s/Fof",sample_prefix);
	snprintf(bgc_file,MAXLEN,"/scratch/02497/jenpi/halo_files/%s/Fof/%s_%d_z%s_fof_b0p2.0000.bgc",sample_prefix,sample_prefix,box,ztag);
