...
int main()
{
        ...
        double chi2_fof(int,int ,double ,double ,double ,double ,double ,double ,double );
        int i=0;
        double chi2;


        for(i=0;i<9;i++){
                chi2= chi2_fof(18,0,i*1.0,0.19, 9.81,12.7053039145 ,  1.14827081243 ,  0.255258314714 , 1.31705259849); 
...
double chi2_fof(int lum_sample,int bender,double box_choose,double siglogM,double logM0,double logM1,double alpha,double gamma,double fgal)
{
	...
	//halobias chars	
	snprintf(halobias_path,MAXLEN,"/home/piscioja/Bin/halobias_fof_nfw_ff");
...
//	snprintf(file_tag,MAXLEN,"tag");
	if(sizeof(file_tag) > MAXLEN)
	  return BAD_VALUE;
	//makemock chars
	snprintf(makemock_path,MAXLEN,"/home/piscioja/Bin/makemock_double");
...
//	snprintf(map_file,MAXLEN,"%s/lss_geometry.dr72.stripe_trim.pix",map_directory);
	...
	snprintf(data_file,MAXLEN,"%s/Wtheta/Wtheta_vollim_Mr%d_fib0.100rand.overlap.filter",data_directory,lum_sample);
        snprintf(covariance_file,MAXLEN,"%s/Wtheta/CovarWtheta_vollim_Mr%d_fib0.100rand.overlap.filter.inv",data_directory,lum_sample);
        snprintf(rand_directory,MAXLEN,"%s/Randoms",data_directory);

	snprintf(randoms_file,MAXLEN,"%s/Wtheta_sdssmock_gamma_main%d.rand_100x_sphere.filter.wtheta",rand_directory,lum_sample);
...
//        fprintf(stderr,"Reading in model file %s\n",model_file);
	...
	while(fscanf(fp4,"%d %lf %lf %lf",&itrash,&trash,&theory_wtheta[count],&trash)!=EOF)
		...
		fprintf(stderr,"%lf %lf\n",datatheta[count],theory_wtheta[count]);	
		count++;
        }
