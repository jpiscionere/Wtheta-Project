...

{
  ...
  sscanf(argv[15],"%lu",&seed) ; // This seed is for choosing number of galaxies
...
/*---Read-group-info-in-BGC-files-----------------------------------------------------*/
  ...
  const size_t padding=2*sizeof(int);//record-markes -> before and after record.
  ...
  ftread(&redshift,sizeof(float),1,stdin) ;
  abox=1.0/(1.0 + redshift);
   

  Nhalos=idat[1];
  Lbox=fdat[0];
  fprintf(stderr,"Nhalos = %d Lbox =%lf Redshift= %lf\n",Nhalos,Lbox,redshift);
  
  Rvir=(double *) calloc(Nhalos,sizeof(double)) ;
  Mhalo=(double *) calloc(Nhalos,sizeof(double)) ;
  Npart=(long *) calloc(Nhalos,sizeof(long)) ;
  ...
  junki=(int *) calloc(Nhalos,sizeof(int)) ;
  junkf=(double *) calloc(Nhalos,sizeof(double)) ;
  



  ftread(junki,sizeof(*junki), Nhalos, stdin);

  ftread(Npart,sizeof(*Npart), Nhalos, stdin);

  ftread(junkf,sizeof(*junkf),Nhalos,stdin);  





  ftread(xcen,sizeof(double),Nhalos,stdin);

  ftread(ycen,sizeof(double),Nhalos,stdin);

  ftread(zcen,sizeof(double),Nhalos,stdin);

  ftread(vxcen,sizeof(double),Nhalos,stdin);

  ftread(vycen,sizeof(double),Nhalos,stdin);

  ftread(vzcen,sizeof(double),Nhalos,stdin);
...
/*---Apply-Warren-et-al-correction-to-mass---*/
  ...
  for(i=0;i<Nhalos;i++){
      ...
      Mhalo[i] = mp*((double)Npart[i]*(1.-pow(((double)Npart[i]),-0.6))) ;
...
//  fprintf(stderr,"halobias> Writing galaxy file. Ngal = %d\n",Ngal) ;
  ...
  ftwrite(xg,sizeof(double),Ngal,stdout);
  ftwrite(yg,sizeof(double),Ngal,stdout);
  ftwrite(zg,sizeof(double),Ngal,stdout);
  ftwrite(vxg,sizeof(double),Ngal,stdout);
  ftwrite(vyg,sizeof(double),Ngal,stdout);
  ftwrite(vzg,sizeof(double),Ngal,stdout);
