...
/*   @-@
     |
     @-@ 
Normally, you would loose 2, but now just one.  Randomly choose which one you keep and assign the other its redshift
...
Normally you loose 2 but now just 1 
*/
                 if(IDfib[fiblist[0]][0]==2 && IDfib[fiblist[1]][0]==2 && IDfib[fiblist[2]][0]==2)
                    {
                      ...
                      if(ran2(&seed)<=0.25)
                        {
                          fibcollisions[fiblist[3]] = fiblist[2] ;

                        }
                      else if(ran2(&seed)<=0.5)
                        {
                        fibcollisions[fiblist[2]] = fiblist[1] ;

                        }
                      else if(ran2(&seed) <=0.75)
                        {
...
throw out 3 connection and pick randomly between the other 2 connected
...
keep galaxies with 2 connections, randomly choose between the galaxies with three connections (first 2 galaxies in fiblist)
...
*/
			...
			if(ran2(&seed) <= 0.25)
				...
				fibcollisions[fiblist[3]]=fiblist[1];
				}
			else if(ran2(&seed)<=0.75)
				{
				fibcollisions[fiblist[0]]=fiblist[1];
