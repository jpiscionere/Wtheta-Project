...
/*                  if(IDfib[fiblist[0]][0]==3 && IDfib[fiblist[1]][0]==3 && IDfib[fiblist[2]][0]==3)
              ...
              if(indx >=5)
               {
               	fprintf(stderr,"IDfib2 > Large Index = %d, Main Galaxy Id = %d,lss id = %d\n",indx,fiblist[indx-1],gal_id[fiblist[indx-1]]);
		for(j=0;j<indx;j++)
               	 {
 ...
 if(ifib==1)                                  /*---ifib=1-------------*/
    {
      for(i=0;i<Nfile_h;i++)
		...
		while((k=IDfib[i][w])!=-1)
		{
			...
			if(fibcollisions[k]==-1)
			{

				dTheta=acos(X[i] * X[k] + Y[i] * Y[k] + Z[i] * Z[k]);
				if(dTheta < thetamax)
				{
					thetamax = dTheta;
					idmin = k;
...
{
  fprintf(stderr,"===========================================================================\n") ;
  fprintf(stderr,"PROGRAM IDfib hodinput ifib seed zcorrect justpairs >output.dat\n") ;
